...
docker build -t httd_test:v01 -f $HOME/docker_test/

docker run --rm -d -p 80:80 httd_test:v01

echo "Test if the build and the run are succeslfully"

test_run=$(curl localhost:80)
STATS=$(echo $?)
...
test_run_httpd=$(curl localhost:81)
STATS=$(echo $?)
...
test_run_nginx=$(curl localhost:81)
STATS=$(echo $?)
